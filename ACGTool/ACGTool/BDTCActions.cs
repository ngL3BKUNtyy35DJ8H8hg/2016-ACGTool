...
namespace ACGTool
{
    public partial class BDTCAction : Form
    {
        private XmlDocument _myMnuDoc;

        private XmlDocument _mySaBanLastFileDocument;
        private XmlDocument _myBDTCFileDocument;

        //Đường dẫn chứa file .diahinh
        private string _diaHinhPath = "";
        //Đường dẫn chứa file MyMnu.xml
        private string _myMnuPath = "";
        //Đường dẫn thư mục chứa các file .xml kịch bản
        private string _myLastSaBanPath = "";

        //Danh sách các file xml đang dùng trong kịch bản
        private List<string> _xmlFileList = new List<string>();
        ...
        private void treeViewScript_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            else if (treeViewScript.SelectedNode.Level == 2)
            {
                ...
                string[] result = treeViewScript.SelectedNode.Name.Split(new string[] {"[]"}, StringSplitOptions.None);
                string filePath = _diaHinhPath + "\\" + result[1];
        ...
        private void buttonScript_Click(object sender, EventArgs e)
        {
            ...
            //Đọc và kiểm tra file .diahinh để lấy đường dẫn file MyMnu.xml
            if (!CheckDiaHinhFile())
                return;
            ...
            GeneralClass.FormatNode(ref tvNodeMnu);
            ...
            foreach (XmlNode nodeMnuItem in node.ChildNodes)
                if (nodeMnuItem.Name == "MnuItem")
                {
                    ...
                    GeneralClass.FormatNode(ref tvNodeMnuItem);
                    ...
                    foreach (XmlNode nodeScript in nodeMnuItem.ChildNodes)
                        if (nodeScript.Name == "Script")
                        {
                            ...
                            {
                                //Lấy đường dẫn 
                                string filePath = _diaHinhPath + "\\" + nodeScript.Attributes["ScrptFile"].Value.Trim();
                                ...
                                //Lưu lại file xml đang dùng trong kịch bản
                                _xmlFileList.Add(filePath);
                                ...
                                if (!treeViewScript.Nodes.ContainsKey(keyNode))
                                {
                                    ...
                                    {
                                        GeneralClass.FormatLeafNode(ref tvNodeScript);
                                    }
                                    else
                                    {
                                        GeneralClass.FormatLeafNode(ref tvNodeScript, Color.Gray);
                                    }
                                }
                            }
                        }
        ...
        /// <summary>
        /// Đọc tất cả các giá trị thuộc tính trog file .diahinh
        /// </summary>
        /// <returns></returns>
        private bool CheckDiaHinhFile()
        {
            ...
            _diaHinhPath = node.Attributes[key].Value;
            //myCurrentDirectory
            if (!txtFilePath.Text.Contains(_diaHinhPath))
            {
                MessageBox.Show(string.Format("{0}: không đúng thư mục với {1}", key, txtFilePath.Text));
                return false;
            }

            //myLastSaBan="bCOI_COIDKZ_XYZ.last"
            key = "myLastSaBan";
            string lastSaBanPath = _diaHinhPath + "\\" +  node.Attributes[key].Value;
            if (!File.Exists(lastSaBanPath))
            {
                MessageBox.Show(string.Format("{0} không tồn tại", lastSaBanPath));
                return false;
            }

            //Đọc file .Last
            _mySaBanLastFileDocument = new XmlDocument();
            _mySaBanLastFileDocument.Load(lastSaBanPath);

            //Lấy đường dẫn file MyMenu.xml
            XmlNode nodeSaBan = _mySaBanLastFileDocument.DocumentElement;
            _myLastSaBanPath = _diaHinhPath + "\\" + nodeSaBan.Attributes["mySaBanDir"].Value;
            _myMnuPath = _myLastSaBanPath + "\\" + "MyMnu.xml";
            
            if (!File.Exists(_myMnuPath))
            {
                MessageBox.Show(string.Format("{0} không tồn tại", _myMnuPath));
                return false;
            }
        ...
        private void BDTCAction_Load(object sender, EventArgs e)
        {
        ...
        private void btnSaveXml_Click(object sender, EventArgs e)
        {
            ...
            {
                string filePath = _diaHinhPath + "\\" + treeViewScript.SelectedNode.Text;
        ...
        private void listViewNotUsingXml_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        private void CheckXmlFiles()
        {
            ...
            DirectoryInfo di = new DirectoryInfo(_myLastSaBanPath);
            ...
            foreach (FileInfo fileInfo in fileInfoArray)
            {
                ...
                if (_xmlFileList.Contains(fileInfo.FullName))
                {
        ...
        private void btnCheckXmlFiles_Click_1(object sender, EventArgs e)
        {
            //Kiểm tra các file xml có dùng trong kịch bản không
            CheckXmlFiles();
        }
