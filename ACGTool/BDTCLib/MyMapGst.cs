    ...
    public class TabFileInfo
    {
        ...
        public TabFileInfo(string tabFilePath)
        {
            try
            {
                ...
                //Regex r2 = new Regex(@"\((?<Lon>\d+\.\d+), (?<Lat>\d+\.\d+)\) \((?<Width>\d+), (?<Height>\d+)\) Label ""(?<Label>[A-Z]+)""");
                Regex r2 = new Regex(@"\((?<Lon>\d+\.\d+),(\s|)(?<Lat>\d+\.\d+)\) \((?<Width>\d+),(\s|)(?<Height>\d+)\) Label ""(?<Label>.+)""");
                //Regex r2 = new Regex(@"\((?<Lon>\d+\.\d+), (?<Lat>\d+\.\d+)\) \((?<Width>\d+), (?<Height>\d+)\)");
                //Regex r2 = new Regex(@"\((?<Lon>\d+\.\d+), (?<Lat>\d+\.\d+)\)");
                ...
                if (!PointInfoDict.ContainsKey("NW") && PointInfoDict.ContainsKey("Pt 1"))
                {
                    PointInfoDict.Add("NW", PointInfoDict["Pt 1"]);
                    PointInfoDict.Add("SW", PointInfoDict["Pt 2"]);
                    PointInfoDict.Add("NE", PointInfoDict["Pt 3"]);
                    PointInfoDict.Add("SE", PointInfoDict["Pt 4"]);
        ...
        public void BindTabInfo_ListView(ListView listViewGstMap)
        {
            ...
            if (PointInfoDict.ContainsKey("Pt 1"))
            {
                listViewGstMap.Items[1].SubItems.Add("Label trong file .tab là Pt 1, Pt 2, Pt 3, Pt 4");
                listViewGstMap.Items[1].ForeColor = Color.Red;
            }

            //Left Longitude
            item = new ListViewItem("Left Longitude");
            item.SubItems.Add(PointInfoDict["NW"].Lon);
            //==============================================================
