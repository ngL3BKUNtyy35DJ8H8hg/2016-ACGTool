...
namespace HT.MilSymbols
{
    public partial class ucMilSymbols : UserControl
    {private string Status;
        ...
        #region "HT Code"
        //Khoảng cách x giữa 2 object trong picture box
        private int x = 10;
        //Khoảng cách y giữa 2 object trong picture box
        private int y = 16;
        //Kích thước Height của thumbnail trong picture box
        private int h = 40;
        //Kích thước Width của thumbnail trong picture box
        private int w = 40;
        //Số row của picturebox
        private int rowsPerPage = 0;
        //Số col của picturebox
        private int colsPerPage = 0;

        /// <summary>
        /// HT Code: Hàm tính lại số row, column trong picturebox
        /// </summary>
        public void CalculateRowsAndColumns()
        {
            int mPicHeight = this.Height - 24;
            int new_rowsPerPage = (mPicHeight - y) / ((2 * y) + h);
            int new_colsPerPage = this.Width / ((2 * x) + w);
            if ((new_rowsPerPage != rowsPerPage) || (new_colsPerPage != colsPerPage))
            {
        ...
        public ucMilSymbols()
        {
            ...
            this.Load += new System.EventHandler(this.vectShapes_Load);
            ...
            if (All)
            {
                ...
                if (s.List.Count > 0)
                {
                    ...
                    for (int row = 0; row < rowsPerPage; row++)
                    {
                        ...
                        {
                            //Xác định rect của thumbnail và tô màu nền
                            Rectangle mRect = new Rectangle((2 * x) + (col * ((2 * x) + w)), (1 * y) + (row * ((2 * y) + h)), w, h);
                            backG.FillRectangle(new SolidBrush(Color.LightGray), mRect);

                            if (itemsDisplayed < s.List.Count)
                            {
                                Ele obj = (Ele)s.List[itemsDisplayed++];

                                //Thực hiện di chuyển đối tượng
                                int tmpX = (int)(mRect.X / Zoom - this.dx);
                                int tmpY = (int)(mRect.Y / Zoom - this.dy);
                                int tmpstartX = obj.PosX;
                                int tmpstartY = obj.PosY;
                                if (fit2grid & this.gridSize > 0)
                                {
                                    tmpX = this.gridSize * (int)((mRect.X / Zoom - this.dx) / this.gridSize);
                                    tmpY = this.gridSize * (int)((mRect.Y / Zoom - this.dy) / this.gridSize);
                                    tmpstartX = this.gridSize * (int)(obj.PosX / this.gridSize);
                                    tmpstartY = this.gridSize * (int)(obj.PosY / this.gridSize);
                                    s.Fit2grid(this.gridSize);
                                    s.sRec.Fit2grid(this.gridSize);
                                }

                                //Move
                                obj.move(tmpstartX - tmpX, tmpstartY - tmpY);
                                obj.endMoveRedim();
                                if (!obj.AmIaGroup())
                                    s.storeDo("U", obj);
                                //obj.Draw(backG, this.dx, this.dy, this.Zoom);

                                //Di chuyển xong, tiếp tục redim hoặc zoom
                                //Resize
                                tmpX = (int)(mRect.Right / Zoom - this.dx);
                                tmpY = (int)(mRect.Bottom / Zoom - this.dy);
                                tmpstartX = obj.getX1();
                                tmpstartY = obj.getY1();
                                if (obj is Group)
                                {
                                    //((Group)obj).GrpZoomX = 1;
                                    //((Group)obj).GrpZoomY = 1;

                                    //obj.redim(tmpX - tmpstartX, tmpY - tmpstartY, "E");
                                    ////((Group)obj).endMoveRedim();

                                    //obj.redim(tmpX - tmpstartX, tmpY - tmpstartY, "S");
                                ...
                                {
                                    obj.redim(tmpX - tmpstartX, tmpY - tmpstartY, "SE");
                                    obj.endMoveRedim();
                                    if (!obj.AmIaGroup())
                                        s.storeDo("U", obj); 

                                    //obj.UpdateRect(mRect.Left, mRect.Top, mRect.Right, mRect.Bottom);
                                }
                                obj.Draw(backG, this.dx, this.dy, this.Zoom);
                            }
                ...
                #region NO muse button pressed
                {
                    if (this.Option == "select")
                    {
                        if (this.s.sRec != null)
                        {
                            ...
                            this.redimStatus = Enum.GetName(typeof(enumRedimOption), st);
