    ...
    public class SelRectBK : Ele
    {
        ...
        public enumRedimOption isOver(int x, int y)
        {
            ...
            if (!this.AmIaGroup())
            {
                //NW
                if (r.Contains(x, y))
                    return enumRedimOption.NW;
                //SE
                r.X = this.X1 - 2;
                r.Y = this.Y1 - 2;
                if (r.Contains(x, y))
                    return enumRedimOption.SE;
            }
            if (!isaLine)
            {
                //N
                ...
                if (!this.AmIaGroup())
                {
                    ...
                    r.Y = this.Y - 2;
                    if (r.Contains(x, y))
                        return enumRedimOption.NE;
                }
                //E
                r.X = this.X1 - 2;
                r.Y = this.Y - 2 + (this.Y1 - this.Y) / 2;
                if (r.Contains(x, y))
                    return enumRedimOption.E;
                //S
                r.X = this.X - 2 + (this.X1 - this.X) / 2;
                r.Y = this.Y1 - 2;
                if (r.Contains(x, y))
                    return enumRedimOption.S;
                if (!this.AmIaGroup())
                {
                    ...
                    r.Y = this.Y1 - 2;
                    if (r.Contains(x, y))
                        return enumRedimOption.SW;
                }
                //W
                r.X = this.X - 2;
                r.Y = this.Y - 2 + (this.Y1 - this.Y) / 2;
                if (r.Contains(x, y))
                    return enumRedimOption.W;
            }

            if (this.contains(x, y))
                return enumRedimOption.C;
