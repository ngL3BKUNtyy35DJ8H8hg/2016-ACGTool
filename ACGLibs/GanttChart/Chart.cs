...
{
    public enum enumGanttChartTimer
    {
        None,
        Start,
        Stop
    ...
    public partial class Chart : UserControl
    {
        ...
        {
            float span = value - _mProject.Start;
            _mViewport.X = span * this.BarWidth;
        }
        ...
        public void ScrollTo(Task task)
        {
            if (_mChartTaskRects.ContainsKey(task))
            {
                ...
                _mViewport.Y = rect.Top - this.HeaderOneHeight - this.HeaderTwoHeight - this.BarHeight;
            }
        }
        ...
        private void _GenerateModels()
        {
            ...
            foreach (var task in _mProject.Tasks)
            {
                if (!_mProject.AncestorsOf(task).Any(x => x.IsCollapsed))
                {
                    ...
                    //HT Notes: Tạo rectangle của task
                    taskRect = new RectangleF(task.Start * this.BarWidth + (float)this.BarWidth/2, y_coord, task.Duration * this.BarWidth, this.BarHeight);
                    //taskRect = new RectangleF(task.Start * this.BarWidth + this.BarWidth / 2, y_coord, task.Duration * this.BarWidth, this.BarHeight);
                    ////HT Code: Nếu duration = 0 thì cho Width = BarWidth
                    if (task.Duration == 0f)
                    {
                        taskRect.Width = BarWidth;
                    }
                    ...
                    if (task.End > end)
                    {
                        ...
                        extraWidth = this.CreateGraphics().MeasureString(task.Name, this.Font).Width;
                    }
            ...
            _mViewport.WorldWidth = Math.Max(pWidth, end * this.BarWidth + extraWidth + this.BarWidth*5);
        }
        ...
        private void _GenerateHeaders()
        {
            ...
            for (int x = (int)(_mViewport.X - _mViewport.X % this.BarWidth); x < _mViewport.Rectangle.Right; x += this.BarWidth)
            {
                ...
                columns.Add(new RectangleF(x + this.BarWidth / 2, columns_Y, this.BarWidth, _mViewport.Rectangle.Height));
        ...
        private void __NextColumn(ref int current)
        {
            //if (_mProject.TimeScale == TimeScale.Day)
                current = current + 1;
            //else // if (_mProject.TimeScale == TimeScale.Week)
        ...
        private void _DrawHeader(Graphics graphics, Rectangle clipRect)
        {
            ...
            Font font = new Font(family, 8.0f);//, FontStyle.Bold | FontStyle.Italic | FontStyle.Underline);
            
            var e = new HeaderPaintEventArgs(graphics, clipRect, this, font, this.HeaderFormat);
            if (PaintHeader != null)
                PaintHeader(this, e);
            ...
            for (int i = 0; i < info.H2LabelRects.Count; i++)
            {
                ...
                //Vẽ phân cách 0.5s giữa các label
                graphics.DrawLine(new Pen(Brushes.Blue),
                                  new PointF(h2labelrect.Right, (h2labelrect.Top + h2labelrect.Bottom)/2 - 1),
                                  new PointF(h2labelrect.Right, (h2labelrect.Top + h2labelrect.Bottom)/2 + 1));
        ...
        private void __DrawHeaderOne(Graphics graphics, int columnIndex, HeaderPaintEventArgs e, RectangleF h2textrect)
        {
            ...
            //    }
            //}
            //else if (this.TimeScaleDisplay == GanttChart.TimeScaleDisplay.DayOfYear && _mProject.TimeScale == TimeScale.Day) //HT Code
            //{
                // put a year
                //if (columnIndex == 0 || (date.Year != info.Dates[columnIndex - 1].Year))
                //{
                    //var h1label = date.ToString("yyyy");
                    //var columnsinlabel = GetDaysInAYear(date.Year);
                    //h1labelrect = new RectangleF(_mViewport.X, _mViewport.Y, _mViewport.WorldWidth, this.HeaderOneHeight);
                    //var h1textrect = _TextAlignCenterMiddle(graphics, h1labelrect, h1label, e.Font);
                    //graphics.DrawString(h1label, e.Font, e.Format.Color, h1textrect);
                    //graphics.DrawRectangle(e.Format.Border, h1labelrect);
                //}
        ...
        private int _DrawTasks(Graphics graphics, Rectangle clipRect)
        {
            ...
            foreach (var task in _mChartTaskRects.Keys)
            {
                ...
                if (taskrect.Left <= viewRect.Right)
                {
                    ...
                    //if (critical) 
                    //    e = new TaskPaintEventArgs(graphics, clipRect, this, task, row, critical, this.Font, this.CriticalTaskFormat);
                    //else 
                    //    e = new TaskPaintEventArgs(graphics, clipRect, this, task, row, critical, this.Font, this.TaskFormat);

                    e = new TaskPaintEventArgs(graphics, clipRect, this, task, row, false, this.Font, this.TaskFormat);
                    ...
                    if (this.ShowTaskLabels && task.Name != string.Empty)
                    {
                        ...
                        //===================================================
                        //HT Code: hiển thị duration
                        //
                        string duration = ""; //Nếu duration = 0
                        if (task.Duration >= 1) //Nếu duration > 1 thì vẽ thêm label duration bên trong task
                        {
                            duration = string.Format("{0}", (float)task.Duration);
                            //graphics.DrawString(duration, e.Font, Brushes.Gray, txtrect);    

                            var txtrectDuration = _TextAlignCenterMiddle(graphics, taskrect, duration, e.Font);
                            graphics.DrawString(duration, e.Font, Brushes.Yellow, txtrectDuration);

                            //string start = "0";
                            //if (task.Start > 0)
                            //{
                            //    start = string.Format("{0}", (float)task.Start);
                            //}
                            //var txtrectStart = _TextAlignLeftMiddle(graphics, taskrect, start, e.Font, 0);
                            ////txtrectStart.Offset(-txtrectStart.Width - BarWidth, 0);
                            //graphics.FillRectangle(e.Format.ForeFill, txtrectStart);
                            //graphics.DrawString(start, e.Font, Brushes.Yellow, txtrectStart);

                            //string end = "0";
                            //if (task.End > 0)
                            //{
                            //    end = string.Format("{0}", (float)task.End);
                            //}
                            //var txtrectEnd = _TextAlignRightMiddle(graphics, taskrect, end, e.Font, 0);
                            ////txtrectEnd.Offset(taskrect.Width-txtrectEnd.Width, 0);
                            //graphics.FillRectangle(e.Format.ForeFill, txtrectEnd);
                            //graphics.DrawString(end, e.Font, Brushes.Yellow, txtrectEnd);
                        }
        ...
        /// ****************************************
        /// </summary>
        private void DrawPredecessorLines1(Graphics graphics, RectangleF clipRectF, RectangleF prect, RectangleF srect)
        {
            var p1 = new PointF(prect.Left, prect.Top + prect.Height / 2.0f);
            var p2 = new PointF(prect.Left - BarWidth / 2, p1.Y);
            var p3 = new PointF(p2.X, srect.Top + srect.Height / 2.0f);
            var p4 = new PointF(srect.Left, p3.Y);
            var size = new SizeF(Math.Abs(p4.X - p2.X), Math.Abs(p4.Y - p2.Y));
            var linerect = p2.Y < p4.Y
                               ? new RectangleF(p2, size)
            ...
            if (clipRectF.IntersectsWith(linerect))
            {
                ...
                graphics.FillRectangle(Brushes.Black, p4.X - 1.5f, p4.Y - 1.5f, 3, 3);
            }
        }

        /// <summary>
        /// Vẽ PredecessorLines dạng 2
        /// ****************************************
        ///                     P1*---*P2
        ///                           |
        ///           P4*-------------*P3
        ...
        /// ****************************************
        /// </summary>
        private void DrawPredecessorLines2(Graphics graphics, RectangleF clipRectF, RectangleF prect, RectangleF srect)
        {
            var p1 = new PointF(prect.Right, prect.Top + prect.Height / 2.0f);
            var p2 = new PointF(prect.Right + BarWidth / 2, p1.Y);
            var p3 = new PointF(p2.X, srect.Top - BarHeight / 2.0f);
            var p4 = new PointF(srect.Left - BarWidth / 2, p3.Y);
            var p5 = new PointF(p4.X, srect.Top + srect.Height / 2.0f);
            var p6 = new PointF(srect.Left, p5.Y);
            var size = new SizeF(Math.Abs(p2.X - p5.X), Math.Abs(p2.Y - p5.Y));
            var linerect = p5.Y < p2.Y
                               ? new RectangleF(p5, size)
            ...
            if (clipRectF.IntersectsWith(linerect))
            {
                ...
                graphics.FillRectangle(Brushes.Black, p3.X - 1.5f, p3.Y - 1.5f, 3, 3);
                graphics.FillRectangle(Brushes.Black, p4.X - 1.5f, p4.Y - 1.5f, 3, 3);
                graphics.FillRectangle(Brushes.Black, p5.X - 1.5f, p5.Y - 1.5f, 3, 3);
                graphics.FillRectangle(Brushes.Black, p6.X - 1.5f, p6.Y - 1.5f, 3, 3);
            }
        }

        /// <summary>
        /// Vẽ PredecessorLines dạng 3
        /// ****************************************
        ///           P1*---*P2
        ///                 |
        ///               P3*---------------*P4
        /// ****************************************
        /// </summary>
        private void DrawPredecessorLines3(Graphics graphics, RectangleF clipRectF, RectangleF prect, RectangleF srect)
        {
            var p1 = new PointF(prect.Right, prect.Top + prect.Height / 2.0f);
            var p2 = new PointF(prect.Right + BarWidth / 2, p1.Y);
            var p3 = new PointF(p2.X, srect.Top + srect.Height / 2.0f);
            ...
            var linerect = p1.Y < p4.Y
                               ? new RectangleF(p1, size)
        ...
        /// ****************************************
        /// </summary>
        private void DrawPredecessorLines4(Graphics graphics, RectangleF clipRectF, RectangleF prect, RectangleF srect)
        {
            var p1 = new PointF(prect.Left, prect.Top + prect.Height / 2.0f);
            var p2 = new PointF(srect.Left - BarWidth / 2, p1.Y);
            var p3 = new PointF(p2.X, srect.Top + srect.Height / 2.0f);
            var p4 = new PointF(srect.Left, p3.Y);
            var size = new SizeF(Math.Abs(p4.X - p2.X), Math.Abs(p4.Y - p2.Y));
            var linerect = p2.Y < p4.Y
        ...
        /// <summary>
        /// HT Note: Hàm vẽ relation giữa các script
        /// </summary>
        /// <param name="graphics"></param>
                ...
                if (_mChartTaskRects.ContainsKey(precedent))
                {
                    foreach (var d in dependants)
                    {
                        ...
                        if (_mChartTaskRects.ContainsKey(dependant))
                        {
                            ...
                            //HT Note: nếu task precedent bao giờ cũng kết thúc trước task dependant 1 khoảng thời gian nhất định
                            if (precedent.End < dependant.Start)
                            {
                                if (dependant.BeginStatus == enumGanttChartTimer.Start)
                                {
                                    DrawPredecessorLines1(graphics, clipRectF, prect, srect);
                                }
                                else if (dependant.BeginStatus == enumGanttChartTimer.Stop)
                                {
                                    DrawPredecessorLines3(graphics, clipRectF, prect, srect);
                                }
                            }
                            else
                            {
                                //HT Note: nếu task precedent bao giờ cũng bắt đầu trước task dependant
                                if (precedent.Start <= dependant.Start)
                                {
                                    //HT Code: Nếu dependant task bắt đầu bằng lệnh Start(precedent)
                                    if (dependant.BeginStatus == enumGanttChartTimer.Start)
                                    {
                                        DrawPredecessorLines1(graphics, clipRectF, prect, srect);
                                    }
                                    ...
                                    {
                                        DrawPredecessorLines2(graphics, clipRectF, prect, srect);
                                    }
                                }
                                else //HT Code: Nếu precedent bắt đầu sau dependant
                                {
                                    //HT Code: Nếu dependant task bắt đầu bằng lệnh Start(precedent)
                                    if (dependant.BeginStatus == enumGanttChartTimer.Start)
                                    {
                                        DrawPredecessorLines4(graphics, clipRectF, prect, srect);
                                    }
                                    else if (dependant.BeginStatus == enumGanttChartTimer.Stop)
                                    {
                                        DrawPredecessorLines2(graphics, clipRectF, prect, srect);
        ...
        /// <summary>
        /// HT Note: Hàm vẽ thanh task
        /// </summary>
        /// <param name="graphics"></param>
        /// <param name="e"></param>
        /// <param name="task"></param>
        /// <param name="taskRect"></param>
        private void __DrawRegularTaskAndGroup(Graphics graphics, TaskPaintEventArgs e, Task task, RectangleF taskRect)
        {
            ...
            //====================================================================
            //HT Code: vẽ lại thanh task
            //Nếu duration > 0
            if (task.Duration > 0 )
            {
                //Nếu task là MnuItemRef thì tô màu nhạt
                if (task.Name.Contains("MnuItemRef"))
                {
                    graphics.FillRectangle(Brushes.LightGray, taskRect);
            ...
            else //duration = 0 thì tô màu Gray
            {
                //taskRect.Width = BarWidth;
                graphics.FillRectangle(Brushes.Gray, taskRect);
                graphics.DrawRectangle(e.Format.Border, taskRect);
            }
            ...
            if (_mProject.IsGroup(task))
            {
                ...
                if (task.Name.Contains("MnuItemRef"))
                {
                    //HT Note: Nếu hiển thị chi tiết group 
                    if (!task.IsCollapsed)
                    {
                        var rod = new RectangleF(taskRect.Left, taskRect.Top, taskRect.Width, taskRect.Height / 2);
                        graphics.FillRectangle(Brushes.Brown, rod);

                        // left bracket
                        graphics.FillPolygon(Brushes.Brown, new PointF[] {
                                new PointF() { X = taskRect.Left, Y = taskRect.Top },
                                new PointF() { X = taskRect.Left, Y = taskRect.Top + BarHeight },
                                new PointF() { X = taskRect.Left + (float)BarWidth/2, Y = taskRect.Top } });
                        // right bracket
                        graphics.FillPolygon(Brushes.Brown, new PointF[] {
                                new PointF() { X = taskRect.Right, Y = taskRect.Top },
                                new PointF() { X = taskRect.Right, Y = taskRect.Top + BarHeight },
                                new PointF() { X = taskRect.Right - (float)BarWidth/2, Y = taskRect.Top } });
                    }
                    else //HT Note: Nếu không hiển thị chi tiết group 
                    {
                        graphics.FillRectangle(Brushes.LightGray, taskRect);

                        var rod = new RectangleF(taskRect.Left, taskRect.Top, taskRect.Width, taskRect.Height / 2);
                        graphics.FillRectangle(Brushes.Orange, rod);
                ...
                else
                {
                    ...
                    {
                        var rod = new RectangleF(taskRect.Left, taskRect.Top, taskRect.Width, taskRect.Height / 2);
                        graphics.FillRectangle(Brushes.DarkRed, rod);

                        // left bracket
                        graphics.FillPolygon(Brushes.DarkRed, new PointF[] {
                                new PointF() { X = taskRect.Left, Y = taskRect.Top },
                                new PointF() { X = taskRect.Left, Y = taskRect.Top + BarHeight },
                                new PointF() { X = taskRect.Left + (float)BarWidth/2, Y = taskRect.Top } });
                        // right bracket
                    ...
                    {
                        graphics.FillRectangle(Brushes.Green, taskRect);
        ...
        private RectangleF _TextAlignRightMiddle(Graphics graphics, RectangleF rect, string text, Font font, float rightMargin = 0.0f)
        {
            var size = graphics.MeasureString(text, font);
            return new RectangleF(new PointF(rect.Right - size.Width - rightMargin, rect.Top + (rect.Height - size.Height) / 2), size);
        }
    ...
    public struct ChartInfo
    {
        ...
        /// <param name="task"></param>
        public ChartInfo(int row, int dateTime, Task task)
            : this()
        {
            Row = row;
            DateTime = dateTime;
