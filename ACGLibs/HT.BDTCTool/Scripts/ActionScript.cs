...
namespace ConfigBDTC2.Scripts
{
    public class ActionScript
    {
        ...
        private void CreateNodes(ref XmlElement rootNode, XmlDocument xmlDoc, List<PointF> point3DList, string objName, int numOfObjNames, float start, float increment, float duration, int minGroup, int maxGroup, bool isDich_DanCoi)
        {
             //Tao dan ban
            int id = 1;
            float curr = start;
            ...
            int index = 0;
            //Lặp cho đến khi >= duration thì dừng
            while (curr + increment < duration)
            {
                PointF p = point3DList[index++];
                //Nếu đã gọi hết các point sẵn có mà vẫn chưa chạy hết duration thì reset
                if (index == point3DList.Count - 1)
                    index = 0;
                XmlElement actionNode = CreateBombardAction(xmlDoc, p, id, objName, objNameId, curr, isDich_DanCoi);
            ...
            //    //Nếu bằng duration thì dừng lại
            //    if (curr >= duration)
            //    {
            //        break;
            //    }
            ...
            ////Nếu chưa bằng duration quy định thì tiếp tục create node
            //if (curr < duration)
            //{
            //    start = float.Parse(rootNode.LastChild.Attributes["Start"].Value.ToString()) + increment;
            //    CreateNodes(ref rootNode, xmlDoc, point3DList, objName, numOfObjNames, start, increment, duration,
            //                minGroup, maxGroup, isDich_DanCoi);
            //}
        }

        public string CreateBoomXml(string tempFile, List<PointF> point3DList, string objName, int numOfObjNames, float start, float increment, float duration, int minGroup, int maxGroup, bool isDich_DanCoi)
        {
