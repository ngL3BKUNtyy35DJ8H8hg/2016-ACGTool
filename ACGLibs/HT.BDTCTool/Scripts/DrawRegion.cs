...
{
    /// <summary>
    /// Lớp vẽ lưới vị trí tọa độ các boom (tạo script)
    /// </summary>
    public class DrawRegion : DrawRectangle
    {
        ...
        public DrawRegion(AxMapXLib.AxMap map1, Projection3D objProjection, int boom, int row, int col)
        {
            AxMap1 = map1;
            objProjection3D = objProjection;
            numOfRows = row;
            numOfColumns = col;
            numOfBooms = boom;
        ...
        private void FillCell(Graphics g, Rectangle rect, int indexCell)
        {
            ...
            objProjection3D.Convert3DCoord(ref x, ref y, ref mapX, ref mapY, Conversion3DConstants.To3D);
            PointF p3D = new PointF(x, y);
            Point3DList.Add(p3D);

            //Fill rect
            //g.FillRectangle(Brushes.Yellow, rectCell);
            g.FillEllipse(Brushes.Yellow, rectCell);
